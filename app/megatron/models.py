...
class MegatronIntegration(models.Model):
    ...
    platform_type = models.IntegerField(choices=(
        (str(t.value), t.name.title()) for t in PlatformType))  # type: ignore
...
class MegatronChannel(models.Model):
    ...
    megatron_integration = models.ForeignKey(MegatronIntegration, null=True, on_delete=models.CASCADE)
    workspace = models.ForeignKey(CustomerWorkspace, null=True, on_delete=models.CASCADE)
