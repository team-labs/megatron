...
from megatron.models import PlatformUser
...
class SlackConnection(BotConnection):
    ...
    def _broadcast(self, broadcast: dict, user_ids: List[str],
        ...
        for slack_id in user_ids:
            ...
            try:
                response = self._post_to_channel(channel_id, broadcast)
            except MegatronException as ex:
                error = {slack_id: str(ex)}
                errors.append(error)
                continue
    ...
    def create_channel(self, channel_name: str) -> Optional[dict]:
        # Waiting: Ideally this would operate on Slack's 'conversation' api
        # but no 'join or create' method is provided
        data = {
            'token': self.token,
            'name': channel_name,
        }
        response = safe_requests.post(JOIN_CHANNEL_URL, data)
        response_data = response.json()
        if not response_data['ok']:
            if response_data['error'] == 'is_archived':
                selected_channel = self.get_channel_by_name(channel_name)
                if selected_channel:
                    self.unarchive_channel(selected_channel['id'])
