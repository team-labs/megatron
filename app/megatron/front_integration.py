...
def incoming_message(msg_data: dict):
    ...
    platform_user = services.WorkspaceService(
        workspace).get_or_create_user_by_id(user_platform_id)
...
def outgoing_message(user_platform_id: int, workspace_platform_id: int, msg_data: dict):
    ...
    workspace = models.CustomerWorkspace.objects.get(platform_id=workspace_platform_id)
    platform_user = services.WorkspaceService(
        workspace).get_or_create_user_by_id(str(user_platform_id))
...
def _translate_slack_msg_to_front_msg(platform_user: models.PlatformUser, slack_msg: dict) -> dict:
    front_message = {
        'sender': {
            'name': str(platform_user),
            'handle': str(platform_user)
        },
        'subject': str(platform_user),
        'body': slack_msg['text'],
        'attachments': [],
        'metadata': {
            'thread_ref': str(platform_user.id)
        }
    }
    if slack_msg.get('attachments'):
        attachment_text = _translate_slack_attachments(
            slack_msg['attachments'])
        front_message['text'] += attachment_text
    return front_message


def _translate_front_bot_msg(platform_user: models.PlatformUser, slack_msg: dict) -> dict:
    front_message = {
        'sender': {
            'name': 'Teampay Bot',
            'handle': 'teampay_bot'
...
def _translate_front_slack_event(from_user_id: str, to_user_id: str, slack_msg: dict) -> dict:
